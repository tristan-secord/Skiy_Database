  ...
  def unsubscribed
    ...
    if @forward_session
      if @forward_session[:request_type] == 'SEND'
        ...
        @reverse_session = ActiveSession.where('user_id = ? AND friend_id = ? AND status IS NOT NULL AND expiry_date > ? AND request_type = ?', @forward_session[:friend_id], @forward_session[:user_id], Time.now, 'SEND').first
        if @reverse_session
          ...
          if @reverse_session.status != nil 
            ...
            if @SenderSessions.count <= 0 
              ...
              @notifications = PendingNotification.where('user_id = ? AND read = ? AND (expiry IS NULL OR expiry > ?)', @forward_session[:friend_id], false, Time.now)
              User.notify_ios(@forward_session[:friend_id], 'UNSUBSCRIBE_SENDER_ONE', '', @notifications.count, true, {"session_id": @reverse_session[:id]}.as_json)
            end
          end
        else 
